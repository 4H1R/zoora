    ...
    <div className={cn("group/message flex w-full flex-col", isOwn ? "items-start" : "items-end")}>
      ...
      {replyToId && (
        <button
          ...
          className={cn(
            "border-primary/60 bg-muted/60 hover:bg-muted mb-0.5 flex max-w-full flex-col items-start gap-0.5 rounded-lg border-s-2 px-2.5 py-1 text-start transition sm:max-w-[50%]",
            isOwn ? "ms-1" : "me-1"
          )}
        >
      ...
      <div className={cn("flex max-w-full items-center gap-1 sm:max-w-[50%]", isOwn ? "flex-row" : "flex-row-reverse")}>
        {maybeWithMenu(
          <div
            className={cn(
