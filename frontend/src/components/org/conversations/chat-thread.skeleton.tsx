...
export function ChatThreadSkeleton() {
  return (
    <div className="flex flex-1 flex-col justify-end gap-3 overflow-hidden p-4">
      {ROWS.map((row, i) => (
        <div key={i} className={cn("flex items-end gap-2", row.own ? "flex-row-reverse ps-12" : "flex-row pe-12")}>
          {row.own ? <div className="w-8 shrink-0" /> : <Skeleton className="size-8 shrink-0 rounded-full" />}
