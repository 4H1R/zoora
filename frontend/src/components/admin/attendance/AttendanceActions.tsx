...
import { getGetAdminAttendanceQueryKey, useDeleteAdminAttendanceId } from "@/api/admin-attendance/admin-attendance"
...
export function AttendanceActions({ attendance, onEdit }: AttendanceActionsProps) {
  ...
  const handleConfirmDelete = () => {
    if (attendance.id) deleteMutation.mutate({ id: attendance.id })
  }
  ...
  return (
    <div className="flex items-center justify-end gap-0.5 opacity-100 transition-opacity sm:opacity-0 sm:group-hover:opacity-100">
      ...
      <DeleteConfirmDialog
        ...
        resourceName={attendance.user?.name ?? attendance.id ?? ""}
