...
func TestBillingRepository_PriceRoundTrip(t *testing.T) {
	...
	price := factory.NewPlanPrice(domain.PlanPro, domain.BillingIntervalMonthly)
	require.NoError(t, repo.UpsertPrice(ctx, price))

	got, err := repo.FindActivePrice(ctx, domain.PlanPro, domain.BillingIntervalMonthly, domain.CurrencyIRR)
	require.NoError(t, err)
	assert.Equal(t, price.Amount, got.Amount)

	// Upserting again deactivates the old row and keeps a single active one.
	newer := factory.NewPlanPrice(domain.PlanPro, domain.BillingIntervalMonthly, func(p *domain.PlanPrice) {
		p.Amount = 2_000_000
	})
